...
<div class="container">
    ...
    <form>
        ...
        <div class="form-group">
            ...
            <input type="datetime-local" name="end" id="end" class="form-control" value="{{ \Illuminate\Support\Carbon::parse($event->end)->format('Y-m-d\TH:i') }}" readonly>
        </div>
        <div class="form-group">
            <label for="deadline">Deadline</label>
            <input type="datetime-local" name="deadline" id="deadline" class="form-control" value="{{ \Illuminate\Support\Carbon::parse($event->deadline)->format('Y-m-d\TH:i') }}" readonly>
        </div>
        <div class="form-group">
            ...
            @foreach ($users as $user)
                <div class="form-check">
                    <input type="checkbox" name="users[]" value="{{ $user->id }}" class="form-check-input" id="user-{{ $user->id }}"
                        {{ in_array($user->id, $eventUsers) ? 'checked' : '' }} disabled readonly>
